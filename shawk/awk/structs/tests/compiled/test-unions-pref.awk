...
# <types>
function AST_U_SIGNED() {return "^(INT|DOUBLE)$"}
function AST_U_NUM() {return "^(UINT|INT|DOUBLE)$"}
function AST_UA() {return "^(type_a|type_b)$"}
function AST_UB() {return "^(type_a|type_b|type_c)$"}
function AST_UC() {return "^(type_d|type_a|type_b|type_c)$"}
# <type-INT>
function AST_INT() {return "INT"}

function ast_INT_make(val,     _ent) {
	_ent = ast_new("INT")
	ast_INT_set_val(_ent, val)
	return _ent
}

function ast_INT_set_val(ent, val) {
	_ast_type_chk(ent, "^(INT)$")
	_ast_set((ent ".val"), val)
}
function ast_INT_get_val(ent) {
	_ast_type_chk(ent, "^(INT)$")
	return _ast_get((ent ".val"))
}

# <\type-INT>
# <type-DOUBLE>
function AST_DOUBLE() {return "DOUBLE"}

function ast_DOUBLE_make(val,     _ent) {
	_ent = ast_new("DOUBLE")
	ast_DOUBLE_set_val(_ent, val)
	return _ent
}

function ast_DOUBLE_set_val(ent, val) {
	_ast_type_chk(ent, "^(DOUBLE)$")
	_ast_set((ent ".val"), val)
}
function ast_DOUBLE_get_val(ent) {
	_ast_type_chk(ent, "^(DOUBLE)$")
	return _ast_get((ent ".val"))
}
...
# <\type-DOUBLE>
# <type-UINT>
function AST_UINT() {return "UINT"}

function ast_UINT_make(val,     _ent) {
	_ent = ast_new("UINT")
	ast_UINT_set_val(_ent, val)
	return _ent
}

function ast_UINT_set_val(ent, val) {
	_ast_type_chk(ent, "^(UINT)$")
	_ast_set((ent ".val"), val)
}
function ast_UINT_get_val(ent) {
	_ast_type_chk(ent, "^(UINT)$")
	return _ast_get((ent ".val"))
}

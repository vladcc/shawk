...
const char * foo_lex_usr_get_input(void * usr_arg, size_t * out_len);
// user events
...
// <lex_static_inline>
// read input
static inline void foo_lex_get_input(foo_lex_state * lex)
{
	size_t len = 0;
	lex->input = foo_lex_usr_get_input(lex->usr_arg, &len);
	lex->input_start = lex->input;
	lex->input_end_excl = lex->input_start + len;
}

// bump up the line number and zero the line position
static inline void foo_lex_bump_line_num(foo_lex_state * lex)
{
	++lex->input_line_num;
...
static inline int foo_lex_read_ch(foo_lex_state * lex)
{
	if ((lex->curr_ch = *lex->input))
	{
		...
		++lex->input_line_pos;
		if (!(*lex->input))
			foo_lex_get_input(lex);
	}
	return lex->curr_ch;
}

// move the input one position back
// if at the start of the buffer, do nothing
static inline void foo_lex_back_pos(foo_lex_state * lex)
{
	if (lex->input > lex->input_start+1)
	{
		--lex->input;
		--lex->input_line_pos;
		lex->curr_ch = lex->input[-1];
	}
}

// peek character n from the current position
// if out of buffer bounds, return \0
static inline int foo_lex_peek_ch_n(foo_lex_state * lex, int n)
{
	const char * target = (lex->input - 1) + n;
	if (target >= lex->input_start && target < lex->input_end_excl)
		return *target;
	return '\0';
}
...
static inline unsigned int foo_lex_get_input_line_pos(foo_lex_state * lex)
{return lex->input_line_pos;}

// get the number of the current input line
static inline unsigned int foo_lex_get_input_line_num(foo_lex_state * lex)

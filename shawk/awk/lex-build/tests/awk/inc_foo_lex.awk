...
function foo_lex_read_line(    _ln) {
	# Note: the user defines foo_lex_usr_get_line()
	if (_ln = foo_lex_usr_get_line()) {
		_B_foo_lex_line_str = _ln
		split(_B_foo_lex_line_str, _B_foo_lex_input_line, "")
		++_B_foo_lex_line_no
		_B_foo_lex_line_pos = 1
		return 1
	}
	_B_foo_lex_line_pos = length(_B_foo_lex_line_str)+2
	return 0
}
...
function foo_lex_read_ch() {
	...
	if (_B_foo_lex_peek_ch != "" || foo_lex_read_line())
		return _B_foo_lex_curr_ch
	else if ("" == _B_foo_lex_curr_ch)
		--_B_foo_lex_line_pos
	return _B_foo_lex_curr_ch
}
...
function foo_lex_peek_ch_n(pos) {
	pos += (_B_foo_lex_line_pos-1)
	return (pos in _B_foo_lex_input_line) ? _B_foo_lex_input_line[pos] : ""
}

# go one character back; if at the first character on the line, do nothing
function foo_lex_back_pos() {
	if (_B_foo_lex_line_pos > 2) {
		_B_foo_lex_peek_ch = _B_foo_lex_input_line[--_B_foo_lex_line_pos]
		_B_foo_lex_curr_ch = _B_foo_lex_input_line[_B_foo_lex_line_pos-1]
	}
}
...
function foo_lex_get_pos_str(last_tok_txt,    _str, _offs) {
	return foo_lex_pos_str_build(_B_foo_lex_line_str, foo_lex_get_pos(), last_tok_txt)
}
function foo_lex_pos_str_build(line_str, pos, last_tok_txt,    _str, _offs) {
...
# <lex_state>
function foo_lex_state_get() {
	return \
	( \
	_B_foo_lex_line_str          _FOO_LEX_SEP() \
	_B_foo_lex_curr_ch           _FOO_LEX_SEP() \
	_B_foo_lex_curr_ch_cls_cache _FOO_LEX_SEP() \
	_B_foo_lex_curr_tok          _FOO_LEX_SEP() \
	_B_foo_lex_line_no           _FOO_LEX_SEP() \
	_B_foo_lex_line_pos          _FOO_LEX_SEP() \
	_B_foo_lex_peek_ch           _FOO_LEX_SEP() \
	_B_foo_lex_peeked_ch_cache   _FOO_LEX_SEP() \
	_B_foo_lex_saved             _FOO_LEX_SEP() \
	)
}

function foo_lex_state_set(state_str,    _arr_st) {
	split(state_str, _arr_st, _FOO_LEX_SEP())

	_B_foo_lex_line_str          = _arr_st[1]
	split(_B_foo_lex_line_str, _B_foo_lex_input_line, "")
	_B_foo_lex_curr_ch           = _arr_st[2]
	_B_foo_lex_curr_ch_cls_cache = _arr_st[3]
	_B_foo_lex_curr_tok          = _arr_st[4]
	_B_foo_lex_line_no           = _arr_st[5]
	_B_foo_lex_line_pos          = _arr_st[6]
	_B_foo_lex_peek_ch           = _arr_st[7]
	_B_foo_lex_peeked_ch_cache   = _arr_st[8]
	_B_foo_lex_saved             = _arr_st[9]
}
# </lex_state>
...
function foo_lex_init() {
	...
	_B_foo_lex_saved = ""
	_B_foo_lex_input_line[""]
	delete _B_foo_lex_input_line
	foo_lex_read_line()
}
# </lex_init>
...
function _FOO_LEX_SEP() {return "\034"}

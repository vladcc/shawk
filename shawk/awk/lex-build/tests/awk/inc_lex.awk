...
function TOK_FCALL() {return "function call"}
function TOK_ERROR() {return "error"}

function CH_CLS_UNUSED_1() {return 1}
function CH_CLS_SPACE() {return 2}
function CH_CLS_WORD() {return 3}
function CH_CLS_NUMBER() {return 4}
function CH_CLS_LESS_THAN() {return 5}
function CH_CLS_UNUSED_2() {return 6}
function CH_CLS_GRTR_THAN() {return 7}
function CH_CLS_NEW_LINE() {return 8}
function CH_CLS_EOI() {return 9}
function CH_CLS_SLASH() {return 10}
function CH_CLS_UNUSED_3() {return 11}
function CH_CLS_AUTO_1_() {return 12}
function CH_CLS_AUTO_2_() {return 13}
# </lex_constants>
...
# return the character at position pos characters away
# return "" if pos is out of bounds
function lex_peek_ch_n(pos) {
	pos += (_B_lex_line_pos-1)
	return (pos in _B_lex_input_line) ? _B_lex_input_line[pos] : ""
}

# go one character back; if at the first character on the line, do nothing
function lex_back_pos() {
	if (_B_lex_line_pos > 2) {
		_B_lex_peek_ch = _B_lex_input_line[--_B_lex_line_pos]
		_B_lex_curr_ch = _B_lex_input_line[_B_lex_line_pos-1]
	}
}
...
function lex_next() {
	...
	while (1) {
		...
		if (2 == _B_lex_curr_ch_cls_cache) { # CH_CLS_SPACE()
			continue
		} else if (3 == _B_lex_curr_ch_cls_cache) { # CH_CLS_WORD()
			_B_lex_curr_tok = lex_usr_get_word()
		} else if (4 == _B_lex_curr_ch_cls_cache) { # CH_CLS_NUMBER()
			_B_lex_curr_tok = lex_usr_get_number()
		} else if (5 == _B_lex_curr_ch_cls_cache) { # CH_CLS_LESS_THAN()
			_B_lex_curr_tok = "<"
			if ("=" == lex_peek_ch()) {
				lex_read_ch()
				_B_lex_curr_tok = "<="
			} 
		} else if (7 == _B_lex_curr_ch_cls_cache) { # CH_CLS_GRTR_THAN()
			_B_lex_curr_tok = ">"
			if ("=" == lex_peek_ch()) {
				lex_read_ch()
				_B_lex_curr_tok = ">="
			} 
		} else if (8 == _B_lex_curr_ch_cls_cache) { # CH_CLS_NEW_LINE()
			continue
		} else if (9 == _B_lex_curr_ch_cls_cache) { # CH_CLS_EOI()
			_B_lex_curr_tok = TOK_EOI()
		} else if (10 == _B_lex_curr_ch_cls_cache) { # CH_CLS_SLASH()
			_B_lex_curr_tok = lex_usr_handle_slash()
		} else if (12 == _B_lex_curr_ch_cls_cache) { # CH_CLS_AUTO_1_()
			_B_lex_curr_tok = "="
		...
		} else if (13 == _B_lex_curr_ch_cls_cache) { # CH_CLS_AUTO_2_()
			_B_lex_curr_tok = "&"
		} else {

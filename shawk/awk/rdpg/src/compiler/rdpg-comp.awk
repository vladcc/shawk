...
function lex_read_line(    _ln) {
	...
	_B_lex_line_pos = length(_B_lex_line_str)+1
	return 0
}
...
function lex_get_line_num()
{return _B_lex_line_num}
...
function ast_start_set_lhs_lst(ent, lhs_lst) {
	...
	_ast_set((ent ".lhs_lst"), lhs_lst)
}
function ast_start_get_lhs_lst(ent) {
	_ast_type_chk(ent, "^(start)$")
	return _ast_get((ent ".lhs_lst"))
}
...
function ast_lhs_set_rule_lst(ent, rule_lst) {
	...
	_ast_set((ent ".rule_lst"), rule_lst)
}
function ast_lhs_get_rule_lst(ent) {
	_ast_type_chk(ent, "^(lhs)$")
	return _ast_get((ent ".rule_lst"))
}
...
function ast_rule_set_esc_lst(ent, esc_lst) {
	...
	_ast_set((ent ".esc_lst"), esc_lst)
}
function ast_rule_get_esc_lst(ent) {
	_ast_type_chk(ent, "^(rule)$")
	return _ast_get((ent ".esc_lst"))
}
...
function ast_rule_set_sym_lst(ent, sym_lst) {
	...
	_ast_set((ent ".sym_lst"), sym_lst)
}
function ast_rule_get_sym_lst(ent) {
	_ast_type_chk(ent, "^(rule)$")
	return _ast_get((ent ".sym_lst"))
}
...
_is_no_imm_err) {
	...
	if (_is_no_imm_err) {
        ...
        if (SYNC_NAME_CUSTOM_CALLBACK() == sync_nont_cst_type(_lhs))
            emit(_make_ret(_make_call(_make_esc(sync_nont_cst_callback(_lhs)))))
...
function _cg_gen_header(    _opt, _val) {
	...
	if ("" == _opt)
		...
		_val = _opt
	emit(_make_comnt(sprintf("Synchronization (Sync): %s", _val)))

	_opt = opt_imm()
	_val = (_opt != 0) ? 1 : 0
	emit(_make_comnt(sprintf("Immediate error detection (Imm): %d", _val)))
	emit(IR_COMMENT())
}
function _cg_gen() {
	_cg_gen_header()

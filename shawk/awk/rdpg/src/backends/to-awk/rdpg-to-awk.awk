...
print "tok_curr() - returns the current token in the stream. Only with TokHack=1."
print ""
...
function opt_tok_hack_set(n) {_B_to_awk_opt_tok_hack = n}
function opt_tok_hack()      {return _B_to_awk_opt_tok_hack}
...
function _gen_rest_of_public_and_internal(    _i, _end, _set, _nm) {
	...
	emit("}")
	if (opt_tok_hack()) {
		emit("function rdpg_reread_curr_tok() {")
		tinc()
			emit(sprintf("%s = tok_curr()", _V_CURR_TOK()))
		tdec()
		emit("}")
	}

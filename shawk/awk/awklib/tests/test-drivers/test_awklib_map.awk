...
function test_map_set(    _map) {
	at_test_begin("map_set()")

	map_init(_map)
	at_true(!("foo" in _map))
	
	map_set(_map, "foo", "bar")
	at_true("foo" in _map)
...
function test_map_get(    _map) {
	at_test_begin("map_get()")

	map_init(_map)
	at_true(!("foo" in _map))
	at_true("" == map_get(_map, "foo"))
	at_true(!("foo" in _map))
...
function test_map_size(    _map) {
	...
	map_set(_map, "foo", "bar")
	map_set(_map, "zig", "zag")
	map_set(_map, "100", "200")
...
function test_map_copy(    _map_a, _map_b) {
	...
	map_set(_map_a, "foo", "bar")
	map_set(_map_a, "zig", "zag")
	map_set(_map_b, "no", "removed")
	
	at_true(2 == map_copy(_map_b, _map_a))
	
	at_true("foo" == map_get_key(_map_b, "bar"))
	at_true("bar" == map_get(_map_b, "foo"))
	at_true("zig" == map_get_key(_map_b, "zag"))
	at_true("zag" == map_get(_map_b, "zig"))
}
...
function test_map_overlay_new(    _map_a, _map_b, _map_c) {
	...
	map_set(_map_a, "foo", "100")
	map_set(_map_a, "bar", "200")
	map_set(_map_a, "baz", "300")
	map_set(_map_a, "zag", "400")
	
	map_set(_map_b, "foo", "1")
	map_set(_map_b, "zig", "10")
	map_set(_map_b, "zag", "20")
	
	map_set(_map_c, "foo", "1")
	map_set(_map_c, "bar", "200")
	map_set(_map_c, "baz", "300")
	map_set(_map_c, "zig", "10")
	map_set(_map_c, "zag", "20")
...
function test_map_overlay_all(    _map_a, _map_b, _map_c) {
	...
	map_set(_map_c, "foo", "100")
	map_set(_map_c, "bar", "200")
	map_set(_map_c, "baz", "300")
	map_set(_map_c, "zig", "10")
	map_set(_map_c, "zag", "400")
...
function test_map_reverse_once(    _map_a, _map_b, _map_c) {
	...
	map_set(_map_a, "foo", "100")
	map_set(_map_a, "bar", "100")
	map_set(_map_a, "baz", "300")
	
	map_set(_map_c, "100", "foo")
	map_set(_map_c, "300", "baz")
...
function test_map_to_str(    _map, _str) {
	...
	map_init(_map)
	map_set(_map, "foo", "100")
	map_set(_map, "bar", "200")
...
function test_map_is_empty(    _map, _str) {
	at_test_begin("map_is_empty()")

	map_init(_map)
	at_true(1 == map_is_empty(_map))

	map_set(_map, "foo", "100")
	at_true(0 == map_is_empty(_map))
